...
import org.mortbay.ijetty.firstblh.AndroidWebAppDeployer;
...
public class IJettyService extends Service {
    ...
    private static final int NOTIFICATION_FLAG = 1991;
                ...
                switch (msg.getData().getInt("state")) {
                    case __STARTED: {
                        ...
                        PendingIntent pendingIntent = PendingIntent.getActivity(getApplicationContext(), 0,
                                new Intent(getApplicationContext(), IJetty.class), 0);
                        Notification notify2 = new Notification.Builder(getApplicationContext())
                                .setSmallIcon(R.drawable.icon_logo) // 设置状态栏中的小图片，尺寸一般建议在24×24，这个图片同样也是在下拉状态栏中所显示，如果在那里需要更换更大的图片，可以使用setLargeIcon(Bitmap
                                // icon)
                                .setTicker(getResources().getString(R.string.app_name))// 设置在status
                                // bar上显示的提示文字
                                .setContentTitle("推新科技")// 设置在下拉status
                                // bar后Activity，本例子中的NotififyMessage的TextView中显示的标题
                                .setContentText("服务器已经启动，正在运行。。。")// TextView中显示的详细内容
...
//                staticDeployer.setAttribute(ANDROID_CONTEXT_ATTRIBUTE, (Context)IJettyService.this);
                staticDeployer.setAttribute(ANDROID_CONTEXT_ATTRIBUTE, IJettyService.this);
...
//            File templeFile = new File(jettyDir, IJetty.__CONTEXTS_DIR);
...
//                contextDeployer.setAttribute(ANDROID_CONTEXT_ATTRIBUTE, (Context) IJettyService.this);
                contextDeployer.setAttribute(ANDROID_CONTEXT_ATTRIBUTE, IJettyService.this);
                contextDeployer.setContexts(contexts);
            }
//            Log.e("firstblh",""+ templeFile.exists());
            if (server != null) {
